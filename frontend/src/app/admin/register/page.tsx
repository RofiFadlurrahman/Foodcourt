...
export default function AdminRegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dbSimulator.saveUser({
        id: "", // Will be auto-generated in dbSimulator
      ...
      setSuccess("Pendaftaran Admin berhasil! Mengalihkan ke halaman masuk...");
    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 relative overflow-hidden font-sans transition-colors duration-300">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <Link href="/admin/login" className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white transition-colors group">
            <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-0.5 transition-transform" /> Kembali ke Masuk Admin
          </Link>
        ...
        <div className="rounded-2xl p-8 border border-slate-200/70 bg-white/80 shadow-2xl backdrop-blur-xl relative overflow-hidden dark:border-white/10 dark:bg-slate-900/70">
          ...
          <form onSubmit={handleRegister} className="space-y-3.5">
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-100/80 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all dark:bg-slate-950/60 dark:border-white/5 dark:text-slate-100"
                  disabled={loading}
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-100/80 border border-slate-200 rounded-xl pl-10 pr-10 py-2.5 text-sm text-slate-900 placeholder-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all dark:bg-slate-950/60 dark:border-white/5 dark:text-slate-100"
                  disabled={loading}
                />
                <button
                  type="button"
            ...
            <button
              type="submit"
              className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-indigo-600/20 flex items-center justify-center gap-2 hover:scale-[1.01] active:scale-[0.99] disabled:opacity-50 disabled:pointer-events-none mt-2"
              disabled={loading}
            >
              {loading ? (
                <>
