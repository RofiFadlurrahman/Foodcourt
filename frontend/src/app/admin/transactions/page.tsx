...
import { Trash2, Search, Printer, CheckCircle, FileSpreadsheet } from "lucide-react";
...
export default function AdminTransactions() {
  ...
  const fetchData = async () => {
    try {
      ...
      setTransactions(txList.sort((a, b) => b.tanggal_transaksi.localeCompare(a.tanggal_transaksi)));
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Apakah Anda yakin ingin membatalkan/menghapus transaksi ini?")) {
      try {
        setLoading(true);
        await dbSimulator.deleteTransaction(id);
  ...
  const filteredTxs = transactions.filter((tx) => {
    ...
    const matchesSearch = tx.id.toLowerCase().includes(searchQuery.toLowerCase()) || menuName.toLowerCase().includes(searchQuery.toLowerCase());
    const matchesTenant = selectedTenantFilter === "all" || tx.tenant_id === selectedTenantFilter;
    const matchesMethod = selectedMethodFilter === "all" || tx.metode_pembayaran === selectedMethodFilter;

    return matchesSearch && matchesTenant && matchesMethod;
  });
  ...
  const handleExportExcel = () => {
    ...
    // Header
    let csvContent = "data:text/csv;charset=utf-8,ID Transaksi,Tenant,Menu,Jumlah,Total Harga,Metode Pembayaran,Tanggal\n";

    filteredTxs.forEach((tx) => {
      const tName = tenants.find((t) => t.id === tx.tenant_id)?.nama_tenant || "Tenant";
      ...
      csvContent += `"${tx.id}","${tName}","${mName}",${tx.jumlah},${tx.total_harga},"${tx.metode_pembayaran}","${formattedDate}"\n`;
    });

    const encodedUri = encodeURI(csvContent);
    const link = document.createElement("a");
    link.setAttribute("href", encodedUri);
    link.setAttribute("download", `Laporan_Transaksi_Foodcourt_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <DashboardLayout roleRequired="admin">
      <div className="space-y-6 print:p-0 print:m-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 print:hidden">
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight">Riwayat Transaksi Penjualan</h1>
            <p className="text-slate-500 text-xs sm:text-sm mt-0.5">Daftar transaksi audit global dari seluruh outlet kuliner foodcourt.</p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Printer className="w-4 h-4 text-indigo-500" /> Cetak (Print)
            </button>
        ...
        <div className="hidden print:block text-center space-y-2 border-b border-slate-300 pb-4 mb-6 font-mono">
          <h2 className="text-xl font-extrabold">LAPORAN TRANSAKSI PENJUALAN CLOUDFOOD</h2>
          <p className="text-xs">Dicetak pada: {new Date().toLocaleString("id-ID")}</p>
          <p className="text-[10px] text-slate-500">
            Jumlah Transaksi: {filteredTxs.length} | Total Omzet: {formatRupiah(filteredTxs.reduce((a, b) => a + b.total_harga, 0))}
          </p>
        </div>

        {/* Filters */}
        <div className="bg-white dark:bg-[#0d1222] border border-slate-200 dark:border-slate-800/40 rounded-2xl p-4 flex flex-col md:flex-row md:items-center gap-4 justify-between print:hidden">
          ...
          <div className="text-xs text-slate-500 font-bold">
            Omzet Terfilter: <span className="text-indigo-500">{formatRupiah(filteredTxs.reduce((acc, curr) => acc + curr.total_harga, 0))}</span>
          </div>
        </div>
              ...
              <table className="w-full text-left border-collapse text-xs sm:text-sm">
                <thead>
                  <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-400 uppercase tracking-wider font-bold text-[10px]">
                    ...
                    <th className="py-3">Tenant Outlet</th>
                    <th className="py-3">Hidangan</th>
                    <th className="py-3">Jumlah</th>
                    <th className="py-3">Total Harga</th>
                    <th className="py-3">Metode</th>
                    <th className="py-3">Tanggal Waktu</th>
                    <th className="py-3 text-right print:hidden">Tindakan</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40 text-slate-700 dark:text-slate-300">
                  {filteredTxs.map((tx) => {
                    const matchedTenant = tenants.find((t) => t.id === tx.tenant_id);
                    const matchedMenu = menus.find((m) => m.id === tx.menu_id);
                    return (
                      <tr key={tx.id} className="hover:bg-slate-100/50 dark:hover:bg-slate-800/10">
                        ...
                        <td className="py-3.5 font-semibold text-slate-800 dark:text-slate-200">{matchedTenant ? matchedTenant.nama_tenant : "Tenant"}</td>
                        <td className="py-3.5 font-semibold">{matchedMenu ? matchedMenu.nama_menu : "Hidangan"}</td>
                        ...
                        <td className="py-3.5">
                          <span
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                              tx.metode_pembayaran === "QRIS" ? "bg-cyan-500/15 text-cyan-400" : tx.metode_pembayaran === "Debit" ? "bg-indigo-500/15 text-indigo-400" : "bg-slate-500/15 text-slate-400"
                            }`}
                          >
                        ...
                        <td className="py-3.5 text-right print:hidden">
                          <button onClick={() => handleDelete(tx.id)} className="p-1.5 text-rose-400 hover:bg-rose-500/10 rounded-lg border border-slate-200 dark:border-slate-800" title="Batalkan / Hapus Transaksi">
