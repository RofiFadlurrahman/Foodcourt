...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $namaTenantInput = trim($_POST['nama_tenant'] ?? '');
    
    if ($usernameInput !== '' && $passwordInput !== '' && $confirmPasswordInput !== '' && $namaTenantInput !== '') {
        if ($passwordInput !== $confirmPasswordInput) {
            ...
            try {
                ...
                if ($stmtCheck->fetchColumn() > 0) {
                    ...
                    $stmtUser = $pdo->prepare("INSERT INTO `users` (`username`, `password`, `role`) VALUES (?, ?, 'tenant')");
                    $stmtUser->execute([$usernameInput, $hashedPassword]);
                    
                    $userId = $pdo->lastInsertId();
                    
                    // 2. Simpan profil tenant
                    $stmtTenant = $pdo->prepare("INSERT INTO `tenants` (`user_id`, `nama_tenant`) VALUES (?, ?)");
                    $stmtTenant->execute([$userId, $namaTenantInput]);
                    
                    // Commit transaksi
                    $pdo->commit();
                    
                    $success = 'Pendaftaran Tenant berhasil! Mengalihkan ke halaman masuk...';
                }
...
<head>
    ...
    <title>Daftar Tenant - Sistem Foodcourt</title>
    <!-- Bootstrap 5 CSS CDN -->
...
<body>
    ...
    <div class="login-card text-center" style="max-height: 95vh; overflow-y: auto;">
        <div class="logo-box">
        ...
        <h3 class="text-title">Daftar Tenant</h3>
        <p class="text-subtitle">Registrasi outlet/tenant kuliner baru</p>
        ...
        <form action="" method="POST" novalidate>
            <div class="form-group text-start mb-3">
                <label for="nama_tenant" class="form-label mb-1">Nama Tenant / Outlet</label>
                <input type="text" class="form-control" id="nama_tenant" name="nama_tenant" placeholder="Contoh: Kopi Kenangan" required>
            </div>
            <div class="form-group text-start mb-3">
        ...
        <div class="mt-2 text-center" style="font-size: 13px;">
            <span class="text-muted">Sudah punya akun tenant? </span>
            <a href="login.php" class="text-decoration-none" style="color: #6ee7b7; font-weight: 600;">Masuk di sini</a>
        </div>
    </div>
